...
function RecoverInformation() {
    ...
    const anotherSuper = document.querySelector('#newSuperInput').value;
    ...
    if (typeSuper === "Otro") {
        anotherNewSuper = `%0A%0A*Super Agregado:*%0A${anotherSuper}`;
    }

    const mensaje = `send?phone=${telefono}&text=*ENVIADO DESDE LA WEB*%0A%0A*Tipo de servicio:*%0AMercadito%0A%0A${encodeURIComponent(productInfo)}*Mercado seleccionado:*%0A${typeSuper}${anotherNewSuper}%0A%0A*Dirección de entrega:*%0A${direction}%0A%0A*Whats'App:*%0A${whatsapp}%0A%0A*Forma de pago:*%0A${payDelivery}%0A%0A*Tipo de pedido:*%0A${typedelivery}${additionalInfo}`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const typeSuper = document.querySelector('#typeSuper');
    const anotherSuper = document.querySelector('#newSuper');
    const inputSuper = document.querySelector('#newSuperInput');

    typeSuper.addEventListener("change", function () {
        const selectedValue = typeSuper.value;
    ...
    function funcionSuper() {
        anotherSuper.style.display = 'none';
    }

    function funcionAnotherSuper() {
        anotherSuper.style.display = 'grid';
        inputSuper.style.height = '51px';
    }
});