...
function RecoverInformation() {
    ...
    const numberAcount = document.querySelector('#numberAcount').value;
    const nameComplete = document.querySelector('#nameComplete').value;
    ...
    const typeBank = document.querySelector('#typeBanck').value;
    ...
    const mensaje = `send?phone=${telefono}&text=*ENVIADO DESDE LA WEB*%0A%0A*Tipo de servicio:*%0ADeposito Bancario%0A%0A*Monto a depositar:*%0A${amount}%0A%0A*Numero de cuenta:*%0A${numberAcount}%0A%0A*Banco donde realizara el deposito:*%0A${typeBank}%0A%0A*Nombre Completo:*%0A${nameComplete}%0A%0A*Dirección/Ubicacion Actual:*%0A${direction}%0A%0A*Whats'App:*%0A${whatsapp}%0A%0A*Forma de pago:*%0A${payDelivery}%0A%0A*Tipo de pedido:*%0A${typedelivery}${additionalInfo}`;
